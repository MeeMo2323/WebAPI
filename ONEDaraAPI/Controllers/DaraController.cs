...
namespace ONEDaraAPI.Controllers.Dara
{
    [ApiController]
    ...
    public class DaraController : ControllerBase
    {
        ...
        [HttpGet("getAllDara")] /*Done*/
        public IActionResult GetAllDara(string U_ID)
        {
            appconfig.ReadAppconfig();
            getRequestData();
            if (checkValidToken(strAuthorization))
            {
        ...
        public IActionResult GetBeUnder()
        {
          ...
          //  if (checkValidToken(strAuthorization))
           // {
                string query = @"select BE_UNDER from MT_BE_UNDER where IS_DEL is null order by be_under";
                DataTable resTable = new DataTable();
                resTable = dbHelper.DBGetData(query).Tables[0];
                string strReturn = JsonConvert.SerializeObject(resTable, Newtonsoft.Json.Formatting.Indented);
                return Ok(strReturn);
        ...
        {
            string query = @"select *,null as isSelected from MT_TAG WHERE IS_DEL is null order by TAG_NAME";
            DataTable resTable = new DataTable();
            resTable = dbHelper.DBGetData(query).Tables[0];
            string strReturn = JsonConvert.SerializeObject(resTable, Newtonsoft.Json.Formatting.Indented);
            return Ok(strReturn);
        ...
        public IActionResult LogIn(User oUser)
        {
           ...
           // string query = @"select U_ID,U_ROLE,U_NAME from [MT_USER] where U_NAME='"+ oUser.U_NAME + "' and U_PASS='"+ oUser.U_PASS + "' and IS_DEL is null";
            string query = @"select a.U_ID,MT_ROLE.U_ROLE,a.U_NAME,a.U_EMAIL
                             ...
                             where a.U_NAME='" + oUser.U_NAME + "' and U_PASSHASH='" + appconfig.passwordEncrypt(oUser.U_PASS,appconfig.ApplicationSecret) + "' and a.IS_DEL is null";
            ...
            if(resTable.Rows.Count>0){
                ...
                result = auth.GenerateToken(resTable.Rows[0]["U_ID"].ToString(), resTable.Rows[0]["U_ROLE"].ToString(), resTable.Rows[0]["U_NAME"].ToString(), Convert.ToInt32(appconfig.DaraTokenValue), appconfig.DaraTokenPeriod);                
                string strReturn = JsonConvert.SerializeObject(new { U_ID = resTable.Rows[0]["U_ID"].ToString(), U_ROLE = resTable.Rows[0]["U_ROLE"].ToString(), U_NAME = resTable.Rows[0]["U_NAME"].ToString(), U_EMAIL = resTable.Rows[0]["U_EMAIL"].ToString(), token = result }, Newtonsoft.Json.Formatting.Indented);
                return Ok(strReturn);
            }
            else
            {
        ...
        private bool checkValidToken(string token)
        {
            MyClass.AppConfigHelper appconfig = new MyClass.AppConfigHelper();
            appconfig.ReadAppconfig();
            string strToken;
        ...
        public IActionResult updateProfile([FromForm] formData oData)
        {
            MyClass.AppConfigHelper appconfig = new MyClass.AppConfigHelper();
            appconfig.ReadAppconfig();

            resExecuted oRes = new resExecuted();
            var objProfile = Newtonsoft.Json.JsonConvert.DeserializeObject<APP_PROFILES>(oData.Data);
            var objUser = Newtonsoft.Json.JsonConvert.DeserializeObject<User>(oData.User);

            string srtResult = "";
            string strSQL = " select count(U_ID) " +
                            " from [WebPortal].[dbo].[MT_USER] " +
                            " where IS_DEL is null and U_ID = " + objProfile.U_ID +
                            " and U_PASSHASH = '"+ appconfig.passwordEncrypt(objProfile.U_PASS, appconfig.ApplicationSecret) + "'";

            srtResult = dbHelper.DBExecuteResult(strSQL);
            if(srtResult=="0")
            {
                oRes.RESULT = "E";
                oRes.MESSAGE = "บันทึกข้อมูลไม่สำเร็จ, รหัสผ่านปัจจุบันไม่ถูกต้อง";
            ...
            else
            {
                try
                {
                    strSQL = "UPDATE [WebPortal].[dbo].[MT_USER] set " + (objProfile.U_EMAIL == null || objProfile.U_EMAIL == "" ? "" : " U_EMAIL='" + objProfile.U_EMAIL + "',") + (objProfile.U_NEWPASS == null || objProfile.U_NEWPASS == "" ? "" : " U_PASSHASH='" + appconfig.passwordEncrypt(objProfile.U_NEWPASS, appconfig.ApplicationSecret) + "',") +
                                                         " UPDATE_DATE = getdate(), UPDATE_BY = '" + objUser.U_NAME + "'" +
                                    "WHERE U_ID = " + objProfile.U_ID;
                    dbHelper.DBExecute(strSQL);
                    oRes.RESULT = "1";
        ...
        [HttpGet("getEncryptText")]
        public IActionResult GetEncryptText(string strValueToEncrypt)
        {
            MyClass.AppConfigHelper appconfig = new MyClass.AppConfigHelper();
            appconfig.ReadAppconfig();
            string strReturn = JsonConvert.SerializeObject(appconfig.passwordEncrypt(strValueToEncrypt,appconfig.ApplicationSecret), Newtonsoft.Json.Formatting.Indented);
            return Ok(strReturn);
        }
    ...
    public class APP_PROFILES
    {
        ...
        public string U_PASS { get; set; }
        public string U_NEWPASS { get; set; }
